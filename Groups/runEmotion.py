...
UNTRAINED_TYPE = "WITHIN"

SUBJECT = sys.argv[1]

OUT_FILE = "./results"

DATA_DIR = "/share/multibranch/datasets/SEED_IV_BU_shuffled/"
...
if (PROFILING):
	CALLBACKS.append(tf.keras.callbacks.TensorBoard(log_dir=PROFILING_OUT_DIR))
...
def load_data(data_dir, num):
	x = np.load(data_dir + "A0" + str(num) + "D_shuffled.npy", encoding='latin1', allow_pickle=True)
	y = np.load(data_dir + "A0" + str(num) + "K_shuffled.npy", encoding='latin1', allow_pickle=True)
	x = list(x)
	y = list(y)
...
if (TYPE == "TRAINED"):
	x_tmp, y_tmp = load_data(DATA_DIR, int(SUBJECT))
	x_subs.append(x_tmp)
	y_subs.append(y_tmp)
...
if (KFOLD):
	...
	for train, test in kfold.split(X_data, Y_data):
		...
		model = EEGNet3D(N, Samples = 50, kernLength = 100, branches = BRANCHES)
		model.compile(loss=loss_functions[0], optimizer=optimizers[0], metrics=['accuracy'])
		history = model.fit(x_train, y_train, epochs=300, batch_size=16, validation_data=(x_val, y_val), verbose=0, callbacks=CALLBACKS)
